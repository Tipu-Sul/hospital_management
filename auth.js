const handleRegistration=(event)=>{
    ...
    if (password===confirm_password){
        ...
        if (/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(password))
        {
            ...
            document.getElementById('error').innerText="Password Minimum eight characters, at least one letter, one number and one special character:"   
        }

    }
    else{
        document.getElementById('error').innerText="confirm_password did not match with password";
        alert("confirm_password did not match")
    }
}
